...
# -*- coding: utf-8 -*-
"""
Handles the 'pp speaker remap' command.
"""
...
class SpeakerRemapCommandHandler(DspAudioSettingCommand[int]):
    """Handles the 'pp speaker remap' command."""
    ...
    def command_str(self) -> str:
        return '{"pos_args":["pp speaker remap"]}'

    def process_parsed_data(self, parsed_data: dict) -> CommandResult[int]:
        """
        Process parsed WAMP response data to extract speaker remap value.

        Args:
            parsed_data: The parsed JSON data from WAMP response

        Returns:
            CommandResult[int]: 성공 시 추출된 스피커 리맵 값, 실패 시 에러 메시지
        """
        # pos_args에서 첫 번째 값 추출
        if "pos_args" in parsed_data and isinstance(parsed_data["pos_args"], list):
            if len(parsed_data["pos_args"]) > 0:
                try:
                    speaker_remap_value = int(parsed_data["pos_args"][0])
                    return CommandResult.success(speaker_remap_value)
                except (ValueError, TypeError) as e:
                    return CommandResult.failure(
                        f"Failed to convert speaker remap value to int: {parsed_data['pos_args'][0]}"
                    )
            else:
